...
const EditableTable: FC<TableProps> = ({
  ...
  const handleEditColumnTitle = (key: string, newTitle: string) => {
    const updatedCols = cols.map(column => {
      if (column.key === key) {
        const newKey = DOMPurify.sanitize(newTitle).toLowerCase().replace(/\s/g, '_')
  ...
  const handleAddColumn = () => {
    ...
    const filter = cols.filter(_ => _.title.toLowerCase() == newTitle?.toLowerCase()).length > 0 ? true : false
    if (!newTitle || filter) return alert('You have entered empty value or this value already exists')
    const newKey = DOMPurify.sanitize(newTitle).toLowerCase().replace(/\s/g, '_')
  ...
  const handleSave = () => {
    if (recordId) {
      ...
      setDataTable([...data, { id: Date.now().toString(), header: cols, rows: rows }])
    }
              ...
              <Table>
                        ...
                        <Box display={'flex'} alignItems={'center'} justifyContent={'space-between'}>
                          ...
                          <Box display={'flex'} alignItems={'center'} gap={'5px'}>
                            <Image
                              ...
                              src='/pencil.svg'
                              alt='edit'
                              onClick={() => handleEditColumnTitle(column.key, prompt('Enter new title') || '')}
                            />
                ...
                <Tbody>
                  {rows.map((row, rowIndex) => (
                    <Tr key={rowIndex} fontSize={{ base: '12px', sm: '12px', md: '13px', xl: '14px' }}>
                      {cols.map(column => (
                        <Td key={column.key}>
                          {editingRowId === row.id ? (
                            <Input
                              ...
                              onChange={e =>
                                setRows(rows.map(r => (r.id === row.id ? { ...r, [column.key]: e.target.value } : r)))
                              }
                            />
                          ) : (
                            row[column.key]
                      ...
                      <Td display={'flex'} alignItems={'center'} justifyContent={'center'} gap={'8px'}>
                        ...
                        <Image {...iconStyle} src='/delete.svg' alt='delete' onClick={() => handleDeleteRow(row.id)} />
                      </Td>
                    </Tr>
                  ))}
                </Tbody>
